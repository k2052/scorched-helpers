  ...
  module Helpers
    module Output
      def self.included(base) # @private
        base.send(:include, ScorchedCurrentEngine) unless base.method_defined?(:current_engine)
      end

      ##
      # Module used to detect the current engine in vanilla sinatra apps.
      # @private
      module ScorchedCurrentEngine
        attr_reader :current_engine

        def render(string_or_file, options = {}, &block) # @private
          engine   = derive_engine_symbol(string_or_file)
      ...
      protected
        ...
        def find_proper_handler
          Output.handlers.map { |h| h.new(self) }.find { |h| h.engines.include?(current_engine) && h.is_type? }
        end
    end
  end
end
